...
Testing Final selection : Ridge both 
...
#MSE increases to 97.18, RMSE rose to 9.86, suggesting the model doesnt generalize well to unseen data (overfit)
```

linear model both testing
```{r}
# Prepare the testing data (convert to matrix, same features used for training)
X_test <- as.matrix(testing[, c("tkvht_base", "geom1", "geom2", "gabor1", "gabor2", "gabor3", "gabor4", "gabor5", 
                                "glcm1", "glcm2", "txti1", "txti2", "txti3", "txti4", "txti5", "lbp1", "lbp2", "lbp3", "lbp5")])
# Use the linear (Both) model to make predictions on the testing data
both_lm_pred_test <- predict(model_both_lm, newx = X_test)

# Calculate MSE (Mean Squared Error) for linear (Both) on the testing data
...
# Output the results
cat("Final Evaluation Metrics for Linear (BOTH) Model on Testing Data:\n")
cat("MSE (Mean Squared Error): ", round(mse_both_lm_test, 4), "\n")
cat("RMSE (Root Mean Squared Error): ", round(rmse_both_lm_test, 4), "\n")


# Create a data frame for the final evaluation metrics
final_metrics <- data.frame(
  Metric = c("MSE (Mean Squared Error)", "RMSE (Root Mean Squared Error)"),
  Value = c(round(mse_both_lm_test, 4), round(rmse_both_lm_test, 4))
)

kable(final_metrics, caption = "Final Evaluation Metrics for Linear (BOTH) Model on Testing Data", digits = 4)
```

Linear regression with just tkvht_base:
```{r,echo=FALSE}

# Prepare the testing data (convert to matrix, same features used for training)
X_test <- as.matrix(testing[, "tkvht_base"])

# Use the Ridge (Both) model to make predictions on the testing data
tkv_lm_pred_test <- predict(model_tkv_lm, newx = X_test)
...
# Output the results
cat("Final Evaluation Metrics for Linear (TKVHT) Model on Testing Data:\n")
cat("MSE (Mean Squared Error): ", round(mse_tkv_lm_test, 4), "\n")
cat("RMSE (Root Mean Squared Error): ", round(rmse_tkv_lm_test, 4), "\n")


# Create a data frame for the final evaluation metrics
final_metrics <- data.frame(
  Metric = c("MSE (Mean Squared Error)", "RMSE (Root Mean Squared Error)"),
  Value = c(round(mse_tkv_lm_test, 4), round(rmse_tkv_lm_test, 4))
)

kable(final_metrics, caption = "Final Evaluation Metrics for Linear (TKVHT) Model on Testing Data", digits = 4)

#MSE is only 81.36, RMSE rose to 9.02, suggesting the model doesnt generalize well to unseen data (overfit)

```
Ridge image 
Linear regression with ridge regularization on testing set 
```{r}

# Prepare the testing data (convert to matrix, same features used for training)
X_test <- as.matrix(testing[, c("geom1", "geom2", "gabor1", "gabor2", "gabor3", "gabor4", "gabor5", 
                                "glcm1", "glcm2", "txti1", "txti2", "txti3", "txti4", "txti5", "lbp1", "lbp2", "lbp3", "lbp5")])

# Use the Ridge (Image) model to make predictions on the testing data
ridge_image_pred_test <- predict(ridge_image, s = 0.1, newx = X_test)

# Calculate MSE (Mean Squared Error) for Ridge (Image) on the testing data
...
# Output the results
cat("Final Evaluation Metrics for Ridge (Image) Model on Testing Data:\n")
cat("MSE (Mean Squared Error): ", round(mse_ridge_image_test, 4), "\n")
cat("RMSE (Root Mean Squared Error): ", round(rmse_ridge_image_test, 4), "\n")


# Create a data frame for the final evaluation metrics
final_metrics <- data.frame(
  Metric = c("MSE (Mean Squared Error)", "RMSE (Root Mean Squared Error)"),
  Value = c(round(mse_ridge_image_test, 4), round(rmse_ridge_image_test, 4))
)

kable(final_metrics, caption = "Final Evaluation Metrics for Ridge (Image) Model on Testing Data", digits = 4)

#MSE increases to 102.5841, RMSE rose to 10.1284, suggesting the model doesnt generalize well to unseen data (overfit)

```
Linear regression just image 
model_image_lm
```{r}
# Prepare the testing data (convert to matrix, same features used for training)
X_test <- as.matrix(testing[, c("geom1", "geom2", "gabor1", "gabor2", "gabor3", "gabor4", "gabor5", 
...
# Output the results
cat("Final Evaluation Metrics for Linear (Image) Model on Testing Data:\n")
cat("MSE (Mean Squared Error): ", round(mse_lm_image_test, 4), "\n")
cat("RMSE (Root Mean Squared Error): ", round(rmse_lm_image_test, 4), "\n")


# Create a data frame for the final evaluation metrics
final_metrics <- data.frame(
  Metric = c("MSE (Mean Squared Error)", "RMSE (Root Mean Squared Error)"),
  Value = c(round(mse_lm_image_test, 4), round(rmse_lm_image_test, 4))
)

kable(final_metrics, caption = "Final Evaluation Metrics for Linear (Image) Model on Testing Data", digits = 4)
```
...
logit_both_pred <- predict(model_imaging_log, newdata = training, type = "response")
...
kable(auc_results, caption = "Model Comparison: AUC", digits = 4)
...
kable(auc_results, caption = "Model Comparison: AUC, Accuracy, and F1-Score", digits = 4)

```

Selction: logistic regression (image featuring)

```{r,echo=FALSE}

#make testing predictions on unseen data
test_predictions <- predict(model_imaging_logit, newdata = testing, type = "response")
...
test_roc <- roc(testing$progression, test_predictions)
test_auc <- auc(test_roc)

# Create a performance summary table
performance_summary <- data.frame(
  Metric = c("Accuracy", "Precision", "Recall", "F1-Score", "Specificity", "AUC"),
  Value = c(test_accuracy, test_precision, test_recall, test_f1, test_specificity, test_auc)
)
