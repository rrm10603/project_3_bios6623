...
final_metrics <- data.frame(
  Model = c("Lasso (Both)", "Lasso (Image)", "Ridge (Both)", "Ridge (Image)",  
            "Linear (Both)", "Linear (Image)", "Linear (TKV)"),
  MSE = c(round(mse_lasso_both_test, 4), round(mse_lasso_image_test, 4), round(mse_ridge_both_test, 4), round(mse_ridge_image_test, 4), 
  ...
  RMSE = c(round(rmse_lasso_both_test, 4), round(rmse_lasso_image_test, 4), round(rmse_ridge_both_test, 4), round(rmse_ridge_image_test, 4), 
...
roc_tkvht_base <- roc(training$progression, logit_tkvht_base_pred, direction = "<")
roc_imaging <- roc(training$progression, logit_imaging_pred, direction = "<")
roc_both <- roc(training$progression, logit_both_pred, direction = "<")

roc_lasso_imaging <- roc(training$progression, lasso_logit_imaging_pred, direction = "<")
roc_ridge_imaging <- roc(training$progression, ridge_logit_imaging_pred, direction = "<")

roc_lasso_both <- roc(training$progression, lasso_logit_both_pred, direction = "<")
roc_ridge_both <- roc(training$progression, ridge_logit_both_pred, direction = "<")
...
roc_tkvht_base <- roc(testing$progression, test_tkv_log_pred, direction ="<")
auc_tkvht_base <- auc(roc_tkvht_base)

# 2. Logistic regression model with imaging (imaging model)
roc_imaging <- roc(testing$progression, test_predictions, direction ="<")
auc_imaging <- auc(roc_imaging)

# 3. Logistic regression model with tkvht_base (Base) for test_logit_tkvt
roc_tkvht_logit <- roc(testing$progression, test_logit_tkvt, direction ="<")
auc_tkvht_logit <- auc(roc_tkvht_logit)

# 4. Logistic regression model with both variables (both model)
roc_both_logit <- roc(testing$progression, test_logit_both, direction ="<")
auc_both_logit <- auc(roc_both_logit)

# 5. Lasso regression model on imaging data (Lasso regularization on X_lasso_ti)
roc_lasso_imaging <- roc(testing$progression, test_las_logit_imaging, direction ="<")
auc_lasso_imaging <- auc(roc_lasso_imaging)

# 6. Lasso regression model on both data (Lasso regularization on X_lasso)
roc_lasso_both <- roc(testing$progression, test_las_logit_both_pred, direction ="<")
auc_lasso_both <- auc(roc_lasso_both)

# 7. Ridge regression model on imaging data (Ridge regularization on X_lasso_ti)
roc_ridge_imaging <- roc(testing$progression, test_ridge_imaging_pred, direction ="<")
auc_ridge_imaging <- auc(roc_ridge_imaging)

# 8. Ridge regression model on both data (Ridge regularization on X_lasso)
roc_ridge_both <- roc(testing$progression, test_ridge_logit_both_pred, direction ="<")
auc_ridge_both <- auc(roc_ridge_both)
...
roc_list <- list(
  "Logistic Regression TKHVT" = roc_tkvht_base,
  "Logistic Regression MRI" = roc_imaging,
  "logistic Regression Both" = roc_both_logit,
  "lasso MRI" = roc_lasso_imaging,
  "lasso Both" = roc_lasso_both,
  "ridge MRI" = roc_ridge_imaging,
  "ridge Both" = roc_ridge_both
)
